...
# agent/interactive/refiner.py (文件名简化版)
# -*- coding: utf-8 -*-
...
def _json_diff(old: Dict[str, Any], new: Dict[str, Any], path="$") -> List[str]:
    ...
    if isinstance(old, dict):
        ...
        for k in sorted(keys):
            p = f"{path}.{k}"
            if k not in old: diffs.append(f"{p}: <added>")
            elif k not in new: diffs.append(f"{p}: <removed>")
            else: diffs.extend(_json_diff(old[k], new[k], p))
    elif isinstance(old, list):
        if len(old) != len(new): diffs.append(f"{path}: list len {len(old)} -> {len(new)}")
    elif old != new: diffs.append(f"{path}: '{old}' -> '{new}'")
    return diffs
...
REFINE_SYSTEM = load_template("refiner_system.txt")
...
def refine_prompt_json(base_json_path: str, user_feedback: str, model: str = "deepseek-chat") -> Tuple[Dict[str, Any], Dict[str, Any], List[str]]:
    ...
    user_msg = REFINE_USER_TMPL.format(current_json=json.dumps(old_obj, ensure_ascii=False, indent=2), feedback=user_feedback)
    resp = client.chat.completions.create(model=model, messages=[{"role": "system", "content": REFINE_SYSTEM}, {"role": "user", "content": user_msg}], temperature=0.2, response_format={"type": "json_object"})
    content = resp.choices[0].message.content.strip()
    try: new_obj = json.loads(content)
    except json.JSONDecodeError as e: raise RuntimeError(f"返回不是合法 JSON：\\n{content}") from e
    filtered_obj = _overlay_allowed(old_obj, new_obj)
    try: _ = VideoPromptJSON(**filtered_obj)
    except ValidationError as e: raise RuntimeError(f"改写后的 JSON 不符合 schema：{e}")
